...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by English name, Chinese name (name-zh), or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');const q=data.query.toLowerCase().trim(),raw=data.query.trim();return atlas.concepts.filter(c=>c.name.toLowerCase().includes(q)||c.id.toLowerCase().includes(q)||!!c['name-zh']&&c['name-zh'].includes(raw)).slice(0,30).map(c=>({id:c.id,name:c.name,nameZh:c['name-zh'],pieces:c.elements.length}));}},
